...
export function generateRunningEvaluation(
  ...
  if (analysisType === 'acceleration') {
    ...
    // 最大速度発揮局面の総合評価 - 加速走の場合は専用ラベル
    const topSpeedLabel = isAccelRun ? '加速走' : '最大速度';
    if (avgScore >= 3.5) {
      overallRating = `エリートレベル（${topSpeedLabel}）`;
      if (isAccelRun) {
        overallMessage = '素晴らしい加速走フォームです。助走で得たスピードを効率的に活かし、「短い支持時間で後方までキック」できています。トップスピード維持に最適な動作パターンです。';
      } else {
        overallMessage = '素晴らしい疾走フォームです。「短い支持時間で後方までキック」し、「股関節を用いて素早くキックし、素早く引き付けて」います。世界一流スプリンターの動作特性を備えています。';
      }
    } else if (avgScore >= 3.0) {
      overallRating = `上級レベル（${topSpeedLabel}）`;
      if (isAccelRun) {
        overallMessage = '良好な加速走フォームです。助走の勢いを活かせています。「股関節の伸展速度を高め、膝・足関節の伸展を強調しない」ことで、さらなる速度維持が可能です。';
      } else {
        overallMessage = '良好な疾走フォームです。「股関節の伸展速度を高め、膝・足関節の伸展を強調しない」ことで、さらなる速度向上が期待できます。';
      }
    } else if (avgScore >= 2.5) {
      overallRating = `中級レベル（${topSpeedLabel}）`;
      if (isAccelRun) {
        overallMessage = '加速走フォームに改善の余地があります。助走で得たスピードを維持するため、「脚の屈伸を小さく、脚が流れない（引き付けが素早い）」動作を意識しましょう。';
      } else {
        overallMessage = '疾走動作に改善の余地があります。研究によると、優れたスプリンターは「脚の屈伸が小さく、脚が流れない（引き付けが素早い）」ことが特徴です。「もも上げ」を強調するより、股関節での素早いキックと引き付けを意識しましょう。';
      }
    } else {
      overallRating = `初級レベル（${topSpeedLabel}）`;
      if (isAccelRun) {
        overallMessage = '加速走技術の改善が必要です。助走で得たスピードを維持するため、短い接地時間で効率的に推進力を得る動作を習得しましょう。ほぼ垂直の体幹姿勢（80-90°）を保ち、「真下への踏み込み」を意識してください。';
      } else {
